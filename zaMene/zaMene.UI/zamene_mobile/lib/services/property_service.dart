...
class PropertyService {
  ...
  Future<HomepageRecommendationModel> getHomepageRecommendations(int userId) async {
    final url = Uri.parse('${baseUrl}api/Property/homepage-recommendations/$userId');

    final response = await http.get(url, headers: createHeaders());
    if (response.statusCode == 200) {
      final jsonBody = json.decode(response.body);
      final List<PropertyModel> properties = (jsonBody['properties'] as List)
          .map((item) => PropertyModel.fromJson(item))
          .toList();

      return HomepageRecommendationModel(
        message: jsonBody['message'],
        properties: properties,
      );
    } else {
      throw Exception("Greška pri dohvaćanju preporuka: ${response.statusCode}");
    }
  }
}
